...
Balance should not be assessed on the propensity score itself; not only is this a conceptually invalid way to assess balance [@hoMatchingNonparametricPreprocessing2007] because a propensity score can only be considered useful when it balances the covariates, empirically there is no association between balance on the propensity score and low bias in the effect estimation [@stuartPrognosticScorebasedBalance2013]. The propensity score is sometimes used a heuristic to assess overlap (e.g., to assess whether the failure of propensity score weighting or matching is due to large differences in the propensity score distribution between groups) and to assess why a matching method might be failing (e.g., if nearest neighbor matching cannot even balance the propensity scores, it has little hope for balancing the covariates).
